...
def show_history():
    ...
    if not entries:
        ...
        emby_wins  = sum(1 for e in entries if e.get('winner', '') == 'Emby')
        jelly_wins = sum(1 for e in entries if e.get('winner', '') == 'Jellyfin')
        total      = len(entries)
        tally = (
        ...
        for e in entries:
            ...
            if loser and l_score is not None:
...
def show_main_menu():
    ...
    xbmcplugin.setContent(handle, 'files')
    li = xbmcgui.ListItem(label='Source Battle History')
    li.setArt({'icon': xbmcaddon.Addon().getAddonInfo('icon')})
    xbmcplugin.addDirectoryItem(handle, f"{sys.argv[0]}?action=show_history", li, True)
    xbmcplugin.endOfDirectory(handle)
...
def get_best_source(tmdb_id, imdb_id, tvdb_id, media_type, query, target_year=None, season=None, episode=None):
    ...
    custom_sort = get_int(addon, 'custom_sort', 0)

    custom_4k = addon.getSetting('custom_4k').lower() == 'true'
    custom_dv = addon.getSetting('custom_dv').lower() == 'true'
    custom_audio = addon.getSetting('custom_audio').lower() == 'true'
    ...
    configs = [
        {
            "name": "Emby",
            "url": addon.getSetting('emby_url').rstrip('/').replace(':443', ''),
            "token": addon.getSetting('emby_token'),
            "uid": addon.getSetting('emby_uid')
        },
        {
            "name": "Jellyfin",
            "url": addon.getSetting('jelly_url').rstrip('/').replace(':443', ''),
            "token": addon.getSetting('jelly_token'),
            "uid": addon.getSetting('jelly_uid')
        }
    ]
            ...
            if media_type == 'episode':
                ...
                for id_type, id_val in [(p_tmdb, v_tmdb), (p_tvdb, v_tvdb), (p_imdb, v_imdb)]:
                    ...
                    try:
                        r = session.get(
                            ...
                            f"&Fields=ProviderIds,ProductionYear&Limit=50{user_q}",
                            timeout=15
                        ).json()
                ...
                if v_show_name and v_show_name != query:
                    ...
                    try:
                        r = session.get(
                            ...
                            f"&Fields=ProviderIds,ProductionYear&Limit=20{user_q}",
                            timeout=15
                        ).json()
                        for item in r.get('Items', []):
                            sid = str(item.get('Id', ''))
                ...
                if v_ep_name:
                    try:
                        ...
                        r_d1 = session.get(
                            f"{items_base}?Recursive=True&IncludeItemTypes=Episode"
                            f"&SearchTerm={safe_ep_name}&Fields={fields}&Limit=50{user_q}",
                            timeout=20
                        ).json()
                ...
                if safe_query:
                    try:
                        r_d2 = session.get(
                            f"{items_base}?Recursive=True&IncludeItemTypes=Episode"
                            f"&SearchTerm={safe_query}&Fields={fields}&Limit=100{user_q}",
                            timeout=25
                        ).json()
                ...
                xbmc.log(
                    f"Source Engine [FUNNEL INTAKE]: {s['name']} collected {raw_count} raw "
                ...
                if v_tmdb:
                    try:
                        r = session.get(
                            ...
                            f"&Fields={fields}&Limit=50{user_q}",
                            timeout=15
                        ).json()
                        api_items = r.get('Items', [])
                        if len(api_items) > 10:
                ...
                if v_imdb and not items_to_process:
                    try:
                        r = session.get(
                            f"{items_base}?Recursive=True&IncludeItemTypes=Movie"
                ...
                # SURGICAL MOVIE STRIKE 3: Strict Title & Year match
                if safe_query and not items_to_process:
                    try:
                        r = session.get(
                            f"{items_base}?Recursive=True&IncludeItemTypes=Movie"
                            f"&SearchTerm={safe_query}&Fields={fields}&Limit=50{user_q}",
                            timeout=20
                        ).json()
                        for m in r.get('Items', []):
                            if _titles_match(query, m.get('Name', '')):
                                if target_year and str(target_year).isdigit() and m.get('ProductionYear'):
                                    if abs(int(target_year) - int(m.get('ProductionYear'))) <= 1:
                                        m['match_quality'] = 1.0
                                        items_to_process.append(m)
                                        xbmc.log(
                                            f"Source Engine [MOVIE FOUND]: {s['name']} "
                                            f"'{m.get('Name')}' matched by title+year",
                                            xbmc.LOGWARNING
                                        )
                                else:
                                    m['match_quality'] = 1.0
            ...
            for item in items_to_process:
                ...
                for source in item.get('MediaSources', []):
                    ...
                    if master_preset == 0:
                        ...
                        if custom_sort == 0: score = bitrate_mbps
                        elif custom_sort == 1: score = size_gb
                        else: score = 0

                        if custom_4k and is_4k: score += 50
                        if custom_dv and has_dv: score += 20
                        elif custom_dv and has_hdr: score += 10
                        if custom_audio and (is_atmos_dtsx or is_lossless): score += 20
                        if custom_audio and is_surround: score += 5
...
def play_video():
    ...
    if best:
        ...
        if tie_breaker == 0 and best_other:
        ...
        if best_other:
            ...
            elif master_preset == 5: reason = "Custom Tuning"
        ...
        play_item.setProperty('ResumeTime', str(best['playback_position']))
        play_item.setProperty('TotalTime', str(best['runtime']))

        play_item.setProperty('IsPlayable', 'true')
        play_item.setInfo('video', {'title': query, 'mediatype': media_type})
